# Complete Release Script untuk Cek Picklist
...
Write-Host "🚀 Cek Picklist - Complete Release Script" -ForegroundColor Green
...
$readmeContent = $readmeContent -replace '\*\*Version\*\*: [^\n]+', "**Version**: $newVersionName (Auto-updating)"
$readmeContent = $readmeContent -replace '\*\*Last Updated\*\*: [^\n]+', "**Last Updated**: $currentDate"

# Add release notes if provided
if ($ReleaseNotes -ne "") {
    $releaseSection = "`n## 🆕 **RELEASE NOTES v$newVersionName**`n`n$ReleaseNotes`n`n---`n"
    $readmeContent = $readmeContent -replace '---\s*\n\*\*Version\*\*:', "$releaseSection**Version**:"
}
...
if ($ReleaseNotes -ne "") {
    $commitMessage += "`n`n$ReleaseNotes"
}
